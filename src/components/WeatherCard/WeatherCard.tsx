...
import { useWeatherApi } from '../../actions/weatherAPI';
  ...
  return (
    <div className="weather__item item">
      ...
      <div className="item__temp">
        <WeatherSVG id={weatherRequest.data.weather[0].description} />
        <span>{Math.round(weatherRequest.data.main.temp)}°C</span>
      </div>
      <div className="item__description">
        <p>
          Feels like {Math.round(weatherRequest.data.main.feels_like)}°C. {weatherDescrip}.
        </p>
      </div>
      <ul className="item__details details">
        <li className="details__wind">
          <svg style={{ transform: `rotate(${weatherRequest.data.wind.deg}deg)` }} data-v-47880d39="" width="15px" height="15px" viewBox="0 0 1000 1000" enableBackground="new 0 0 1000 1000">
          ...
          <span>{weatherRequest.data.wind.speed.toFixed(1)}m/s</span>
        </li>
    ...
    -59 -60z"
          ...
          <span>{weatherRequest.data.main.pressure}hPa</span>
        </li>
        <li className="details__humidity">
          <span>Humidity:</span> {weatherRequest.data.main.humidity}%
        </li>
        <li className="details__visability">
          <span>Visibility:</span> {(weatherRequest.data.visibility / 1000).toFixed(1)}km
        </li>
      </ul>
    </div>
  );
